...
const content = fs.readFileSync('C:/Users/Studio3/.openclaw/workspace/nanobanana-prompt-gen/index.html', 'utf8');

const startMarker = '<select id="outfit" onchange="onOutfitChange()">';
const endMarker = '</select>';

const startIdx = content.indexOf(startMarker);
const endIdx = content.indexOf(endMarker, startIdx) + endMarker.length;

if (startIdx === -1) { console.error('Could not find outfit select!'); process.exit(1); }

const oldBlock = content.substring(startIdx, endIdx);

const newOutfits = `<select id="outfit" onchange="onOutfitChange()">
...
const newContent = content.substring(0, startIdx) + newOutfits + content.substring(endIdx);

fs.writeFileSync('C:/Users/Studio3/.openclaw/workspace/nanobanana-prompt-gen/index.html', newContent, 'utf8');

console.log('Done! Replaced ' + oldBlock.length + ' chars with ' + newOutfits.length + ' chars');
