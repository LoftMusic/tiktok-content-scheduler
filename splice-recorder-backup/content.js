...
(function() {
  ...
  // Categories
  const categories = ['kick', 'snare', 'hihat', 'tom', 'clap', 'perc', 'loop', 'fx', 'vocal', 'other'];
  ...
  async function toggleRecording() {
    ...
    if (!isRecording) {
      ...
      const settings = {
        ...
        filename: document.querySelector('#ar-filename')?.value?.trim() || null,
      ...
      chrome.runtime.sendMessage({ type: 'startRecording', settings }, (response) => {
        if (response && response.success) {
          ...
          document.querySelector('#ar-filename').value = '';
        } else {
