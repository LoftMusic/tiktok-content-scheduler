...
(function() {
  ...
  // Loop categories
  const loopCategories = ['fulldrums', 'drumtops', 'kicks', 'snares', 'hihats', 'percussion', 'bass', 'melody', 'fxloop', 'fills', 'vocals'];
  ...
  function detectSampleInfo() {
    ...
    // PRIORITY 1: Look for explicitly focused row first (most reliable)
    ...
    if (row) {
      console.log('[Audio Recorder] Found focused row');
      return extractSampleInfo(row);
    }
    
    // PRIORITY 2: Look for rows that are currently being hovered or have focus-within
    ...
    if (row) {
      console.log('[Audio Recorder] Found focus-within row');
      return extractSampleInfo(row);
    }
    
    // PRIORITY 3: Check for hovered rows (especially in similar sounds modal)
    row = document.querySelector('sp-asset-row:hover, .asset-list-row:hover');
    
    if (row) {
      console.log('[Audio Recorder] Found hovered row');
      return extractSampleInfo(row);
    }
    
    // PRIORITY 4: Look specifically in the similar sounds modal (sp-asset-row elements)
    const similarRows = document.querySelectorAll('sp-asset-row');
    if (similarRows.length > 0) {
      console.log('[Audio Recorder] Found similar sounds, checking all', similarRows.length, 'rows');
      // Try to find the one that's currently playing (has playing class or recently clicked)
      for (const r of similarRows) {
        if (r.classList.contains('playing') || r.classList.contains('active')) {
          return extractSampleInfo(r);
        }
      }
      // Fallback: try the last one (most recently added = most similar to what you clicked)
      if (similarRows.length > 0) {
        return extractSampleInfo(similarRows[similarRows.length - 1]);
      }
    }
    
    // PRIORITY 5: Fallback to main catalog
    const allRows = document.querySelectorAll('[role="row"].asset-list-row, .asset-list-row');
    for (const r of allRows) {
      if (r.matches(':hover, :focus-within') || r.classList.contains('list-focused')) {
        return extractSampleInfo(r);
      }
    }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (playBtn) {
      ...
      // Find the row that contains this play button
      // Walk up the DOM tree to find the row element
      let row = playBtn.closest('sp-asset-row, .asset-list-row, [role="row"]');
      
      if (row) {
        console.log('[Audio Recorder] Found row for clicked play button');
        setTimeout(() => {
          const info = extractSampleInfo(row);
          if (info?.filename) {
            ...
            if (cat) {
              ...
              if (['fulldrums', 'drumtops', 'kicks', 'snares', 'hihats', 'percussion', 'bass', 'melody', 'fxloop', 'fills', 'vocals'].includes(cat)) {
                selectMode('loop');
              } else {
                selectMode('oneshot');
              }
        ...
        // Fallback to detectSampleInfo if we can't find the row
        setTimeout(() => {
